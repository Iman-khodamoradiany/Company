      ...
      <div className="flex flex-col gap-5">
        {Problems.map((item) => (
          <div className="[direction:rtl] text-[#505050] font-mono text-[1.3vw] flex items-center justify-start gap-3">
            <Input className="w-[20px] h-[20px] rounded-[4px]  " type="checkbox" />
            <Text>{item.text}</Text>
          </div>
        ))}
      </div>
      <Input className="bg-[#F9F9F9] w-[465px] h-[100px] rounded-[8px] my-5 mx-auto [direction:rtl] pb-[70px] px-2 text-[#ADADAD] font-bold " placeholder="لطفا نظر خود را درباره این مشاور بنویسید "  />
      <div className="flex items-center justify-center ">
        <Button className="bg-[#CB1B1B] text-white px-[125px] py-[15px] rounded-[8px] my-5  " >ارسال گزارش</Button>
      </div>
    </div>
  );
}
