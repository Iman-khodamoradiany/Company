...
function RealStateDetailsHeader({img}) {
    ...
    const { setheader,setShire,setCard } = useContext(MyContext)
    useEffect(() => {
        ...
        <>
            <header className="text-center flex justify-end items-start h-[100vh] gap-[70px] z-10  flex-col bg-contain lg:bg-cover bg-[url(/Background/Photo2.jpg)] bg-cover bg-no-repeat relative flex  ">
                <section  className="flex  flex-col  w-[50%] h-[45%] gap-8">
                    <Button onClick={() => setShire(true)} className=" bg-white  w-[55%] mx-[115px] h-[260px] rounded-[16px]   flex flex-col">
            ...
            <div className="flex justify-end  w-full h-[120px]  " >
                ...
                <Button onClick={()=>setCard(true)} className=" w-[230px] translate-y-[-65%] mx-[150px] h-[230px] bg-[#F9F9F9] rounded-full  flex justify-center items-center z-50">
                    <Img src={img} className=" text-black" />
                </Button>
            </div>
